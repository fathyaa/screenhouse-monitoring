import { LayoutDashboard, Map, Radio, Leaf, Wifi, CheckCircle, Bell, LogOut, User } from "lucide-react";
...
function Sidebar({ isOpen, screenhouses = [], role = "operator", user }) {
  ...
  const menusByRole = {
    ...
    admin: [
      ...
      { icon: <Leaf size={17} />, label: "Kelola Threshold", path: "/admin/kelola-threshold" },
  ...
  return (
    <aside className={`h-full flex flex-col bg-[#0f2d18] text-white transition-all duration-300 overflow-hidden shrink-0 ${isOpen ? "w-[280px]" : "w-0"}`}>
      ...
      <div className="p-4">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          ...
          <div className="text-xs text-white/50 mt-1 capitalize">{role}</div>
        </div>
      </div>

      {/* MENU */}
      <div className="px-3 flex flex-col gap-0.5">
        <p className="text-[10px] uppercase tracking-widest text-white/30 font-medium px-3 py-2 whitespace-nowrap">Menu</p>
        ...
        {role !== "petani" && (
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-white/5 rounded-xl p-3">
              ...
              <div className="text-2xl font-semibold mt-1">{screenhouses.length}</div>
            </div>
            <div className="bg-white/5 rounded-xl p-3">
              <div className="text-xs text-white/40">Device</div>
              <div className="text-2xl font-semibold mt-1">{screenhouses.length * 4}</div>
            </div>
          </div>
        )}
