-- Data historis 24 jam untuk grafik demo (jalankan setelah seed.sql)
-- Hapus data demo lama opsional, lalu isi ulang untuk screenhouse 1 & 2

DELETE FROM alerts WHERE screenhouse_id IN (1, 2);
DELETE FROM sensor_data WHERE sensor_node_id IN (1, 2, 3);

-- Screenhouse 1 — Node Utara (SH01-N01): pola harian realistis
...
SELECT
  ...
  CASE
    ...
    h BETWEEN 12 AND 16,
    h BETWEEN 7 AND 8 OR h BETWEEN 17 AND 18,
    h < 6 OR h > 18,
    NOW() - (h || ' hours')::interval
FROM generate_series(23, 0, -1) AS h;

-- Screenhouse 1 — Node Selatan: sedikit berbeda (lebih kering siang hari)
INSERT INTO sensor_data (
    sensor_node_id, nitrogen, phosphorus, potassium,
    soil_temperature, soil_moisture, soil_ph, conductivity,
    air_temperature, air_humidity, light_intensity,
...
-- Screenhouse 2 — Node Utama: tren stabil
INSERT INTO sensor_data (
    sensor_node_id, nitrogen, phosphorus, potassium,
    soil_temperature, soil_moisture, soil_ph, conductivity,
    air_temperature, air_humidity, light_intensity,
